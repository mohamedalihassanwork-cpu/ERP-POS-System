...
export function CustomersPage() {
  ...
  const query = useListCustomers({ search: search || undefined, includeInactive, page: 1, pageSize: 100 });
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <label className="flex items-center gap-2 text-sm font-medium text-slate-600 cursor-pointer select-none whitespace-nowrap">
            <input
              ...
              data-testid="checkbox-include-inactive-customers"
            />
            إظهار المعطّلين
          </label>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          {query.isLoading ? (
            ...
            <p className="text-red-500 text-center py-16">تعذّر تحميل العملاء.</p>
          ) : items.length > 0 ? (
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-500">
                <tr>
                  ...
                  <th className="text-right font-bold px-6 py-4">حد الائتمان</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {items.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-500">{money(c.creditLimit)}</td>
                    <td className="px-6 py-4">{balanceBadge(c.currentBalance)}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          ...
                          title="كشف حساب"
                          data-testid={`button-statement-customer-${c.id}`}
                        >
                          <FileText size={16} />
                        </button>
                        {canEdit && (
                          <button
                            ...
                            title="تحصيل دفعة"
                            data-testid={`button-pay-customer-${c.id}`}
                          >
                            <Wallet size={16} />
                          </button>
                        )}
            ...
            <p className="text-slate-400 text-center py-16">لا يوجد عملاء.</p>
          )}
        </div>
      </div>
...
function CustomerModal({
  ...
  async function handle() {
    ...
    if (limit < 0) return setError("حد الائتمان غير صالح.");
    try {
      const data = {
        name: name.trim(),
        phone: phone.trim(),
      ...
      setError(apiErrorMessage(err, "تعذّر حفظ العميل."));
    }
  }

  return (
    <Modal open onClose={onClose} title={isEdit ? "تعديل العميل" : "عميل جديد"}>
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-2">حد الائتمان</label>
          <input
            type="number"
            className={inputClass}
            value={creditLimit}
...
function DeleteCustomerModal({
  ...
  async function handle() {
    setError(null);
    try {
      await mutation.mutateAsync({ id: customer.id });
      onDeleted();
    } catch (err) {
      setError(apiErrorMessage(err, "تعذّر حذف العميل."));
    }
  }
    ...
    <Modal open onClose={onClose} title="تأكيد الحذف">
      <div className="space-y-4">
        <p className="text-slate-600">
          هل تريد حذف العميل{" "}
          <span className="font-bold text-slate-800">{customer.name}</span>؟ سيتم تعطيله.
        </p>
        {error && (
          <div className="bg-red-50 text-red-700 text-sm font-medium rounded-xl px-4 py-3 border border-red-100">
            {error}
...
function CustomerPaymentModal({
  ...
  const mutation = useCreateCustomerPayment();
  ...
  return (
    <Modal open onClose={onClose} title={`تحصيل دفعة — ${customer.name}`}>
      <div className="space-y-4">
        <div className="bg-slate-50 rounded-xl px-4 py-3 text-sm">
          <span className="text-slate-500">الرصيد الحالي: </span>
          <span className="font-bold text-slate-800">{money(customer.currentBalance)}</span>
        </div>
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-2">
...
function CustomerStatementModal({
  ...
  const query = useGetCustomerStatement(customer.id);
  const items = query.data?.items ?? [];

  return (
    <Modal open onClose={onClose} title={`كشف حساب — ${customer.name}`}>
      <div className="space-y-4">
